    ...
    <div className="bg-white rounded-xl px-[20px] py-[40px] md:px-[57px] md:py-[50px] space-y-6 md:space-y-12">
      <div className="grid grid-cols-1 md:grid-cols-2 w-full gap-6 md:gap-10">
        <div className="bg-grey-600 text-grey-500 rounded-xl relative" >
          <LargeInputBox onBlur={()=> setShow(false)} onFocus={()=> setShow(true)} handleChange={formik.handleChange} value={formik.values.school_name} name="school_name" label="School name" iconId="school-icon" placeholder="Enter school name" />
          {show && <div className="absolute w-full bg-white h-[250px] z-[3] rounded-[10px]" >
            <div className="p-1 overflow-y-auto" >
            <div className="flex flex-wrap" >
              {(!isFetching || !isLoading) && data?.data?.map((val: any, index:number) => (<div key={index} className="p-2" >
                <p onMouseDown={()=> handleSelectSearch(val?.name)} className="bg-[#EDEDF2] cursor-pointer rounded-[5px] text-[14px] leading-[22.4px] p-2 w-fit" >{val?.name}</p>
              </div>) )}
            </div>
            </div>
          </div>}
        ...
        <div className="bg-grey-600 text-grey-500 rounded-xl flex gap-4 items-center px-2 md:px-8 py-6 h-full largeselect tab:h-[105px]">
          ...
          <div className="flex flex-col gap-3 w-full">
            ...
            <LargeSelectBox value={formik.values.program_name} matches={matches} name="country_name" placeholder="Enter Country" handleChange={(selectedRole: {label: string, value: string})=> handleSelectChange(selectedRole, "country_name")} options={countryList} />
          </div>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 w-full md:gap-10 gap-6">
        <div className="bg-grey-600 text-grey-500 rounded-xl flex gap-4 items-center px-2 md:px-8 py-6 h-full largeselect tab:h-[105px]">
          ...
          <div className="flex flex-col gap-3 w-full">
            ...
            <LargeSelectBox value={formik.values.program_name} matches={matches} name="program" placeholder="Select Program" handleChange={(selectedRole: {label: string, value: string})=> handleSelectChange(selectedRole, "program")} options={schoolPrograms} />
          </div>
        </div>{" "}
